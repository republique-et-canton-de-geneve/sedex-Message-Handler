/*
 * Copyright (C) 2006-2017 by Bundesamt für Justiz, Fachstelle für Rechtsinformatik
 *
...
/**
 * Signes PDF files, with are located in the configured signing outboxes.
 *
 * @author Kasimir Blaser
 * @author Igor Metz
 */
public class Signer implements ISignerArguments {
   ...
   * @throws org.apache.commons.configuration.ConfigurationException if the signing outboxes are not configured properly
   */
   ...
   *
   * @throws SignerException If something goes wrong..
   * @throws ConfigurationException if the signing outbox is not configured properly
   */
  ...
  private Map<String, String> makeConfigForBatchSigner(SigningOutbox signingOutbox)
  {
    ...
    arguments.put(ACTION, DEFAULT_ACTION);
    arguments.put(P12_FILE, signingOutbox.getP12File().getAbsolutePath());
    arguments.put(P12_PASSWORD, signingOutbox.getPassword());
    arguments.put(SIGNATURE_PROPERTY_FILE, signingOutbox.getSigningProfile().getAbsolutePath());
    arguments.put(CERTIFICATION_TYPE, DEFAULT_CERTIFICATION_TYPE);
    return arguments;
  }

  /**
   * This method must be called after signing the PDF files. If SigningOutbox.getProcessedDir() is set, this method will
   * move all signed PDF files from "signingOutboxDir" to "processedDir". If no processedDir is set, the signed PDFs
   * will be deleted!
   *
   * @param signedFiles List of all files, which have been signed.
   */
    ...
    for(SigningOutbox signingOutbox : signingOutboxes) {
      ...
      if(processedDir == null) {
        ...
        LOG.info("Clean " + signingOutbox.getName() + " after signing: Moved "
                + movedFilesCnt + " PDFs to " + processedDir.getName());
      }
    }
  }

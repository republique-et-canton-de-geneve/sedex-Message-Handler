/*
 * $Id: SenderSession.java 327 2014-01-27 13:07:13Z blaser $
 *
...
public abstract class SenderSession {
	...
	void cleanup(Message message) {
		...
		if (null != message.getEnvelopeFile() && message.getEnvelopeFile().exists() && message.getEnvelopeFile().delete()) {
			LOG.debug(MessageFormat.format("envelope file {0} successfully removed", message.getEnvelopeFile()));
		}

		// data file
		if (null != message.getDataFile() && message.getDataFile().exists() && message.getDataFile().delete()) {
			LOG.debug(MessageFormat.format("data file {0} successfully removed", message.getDataFile()));
		}
	}
	 ...
	 * page 27
	 *
	 * @param message The message to get a receipt from
	 */
	 ...
	 *
	 * @param message The message to pass to SeDeX
